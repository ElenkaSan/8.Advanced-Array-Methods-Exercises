...
function hasOddNumber(arr) {
    const even = arr.some(function(oddNum) {
        return oddNum % 2 !== 0;
    });
    return even;
}
...
function hasAZero(num) {
    const oneZero = num.toString().split('');
    return oneZero.some(function(least){
        return least === '0';
      });
}
...
function hasOnlyOddNumbers(arr) {
    const even = arr.every(function(oddNum) {
        return oddNum % 2 !== 0;
    });
    return even;
 }
...
function hasNoDuplicates(arr) {
      return arr.every(function(dubl) {
        return arr.indexOf(dubl) === arr.lastIndexOf(dubl);
      });
}
...
function hasCertainKey(arr, key) {
    const even = arr.every(function(contains){
        return contains[key];
    });
    return even;
}
...
function hasCertainValue(arr, key, searchValue) {
    const even = arr.every(function(contains){
        return contains[key] === searchValue;
      });
      return even;
}
