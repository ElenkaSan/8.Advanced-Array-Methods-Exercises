...
function showFirstAndLast(arr){
    let twoCharacter = [];
    arr.forEach(function (value) {
        twoCharacter.push(value[0]+value[value.length - 1]);
        })
        return twoCharacter;
}
...
function valTimesIndex(arr){
   const mult = arr.map(function(num, idx) {
        return num * idx;
      });
    return mult;
}
...
// console.log(extractKey([{name: 'Elie'}, {name: 'Tim'}, {name: 'Matt'}, {name: 'Colt'}]));
...
function extractFullName(arr){
    return arr.map(function(nameLastname) {
        return `${nameLastname.first} ${nameLastname.last}`;
      });
}
...
function find(arr, searchValue) {
    const findCorrect = arr.filter(function(value) {
...
function removeVowels(str) {
    const isVowel = str.toLowerCase().split('');
    const noVowel = isVowel.filter (function(char) {
