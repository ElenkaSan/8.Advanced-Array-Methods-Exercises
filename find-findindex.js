...
that matches the string passed to the function. If the object is not found, 
return undefined. 
...
function findUserByUsername(usersArray, username) {
  const arr = usersArray.find(function(string){
...
function removeUser(usersArray, username) {
  const arr = usersArray.findIndex(function(string){
    return string.username === username;
    }); 
    if (arr === -1) return;
