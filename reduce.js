...
function extractValue(arr, key) {
    const takenName = arr.reduce(function(object,name){
...
function vowelCount(str) {
    const object = str.toLowerCase();
    const isVowel= object.split('').reduce(function(take, nextWord) {
        if("aeiou".indexOf(nextWord) !== -1){
            if(take[nextWord]){
                take[nextWord]++;
            } else {
                take[nextWord] = 1;
...
function addKeyAndValue(arr, key, value) {
    const titleName = arr;
    return titleName.reduce(function(object,name) {
        name[key] = value;
        return object;
        }, titleName);
}
...
the callback function on each value in the array and if the result of 
the callback function at that specific value is true, the value should be placed 
in the first subarray. If the result of the callback function at 
...
function partition(arr, callback) {
    const result = arr.reduce(function(called,run){
        if(callback(run)){
            called[0].push(run);
        } else {
            called[1].push(run);
